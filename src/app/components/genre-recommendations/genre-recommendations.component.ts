...
export class GenreRecommendationsComponent {
  ...
  constructor(private fb: FormBuilder, private recommendationService: RecommendationService) {
    this.recommendationForm = this.fb.group({
      ...
      topN: [10, [Validators.required, Validators.min(10), Validators.max(5000)]],
    });
  }
  ...
  getRecommendations(): void {
    if (this.recommendationForm.valid) {
      ...
      this.recommendationService.getGenreRecommendations(selectedGenre, topN).subscribe({
        next: (data) => {
          this.movies = data;
          this.loading = false;
        },
        error: (err) => {
          this.error = err.message || 'Failed to load recommendations for this genre.';
          this.loading = false;
        },
      });
    }
